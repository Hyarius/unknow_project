...
typedef struct		s_mesh
{
	...
	t_vec4			force; // related to kinetic
	float			kinetic;

	t_texture		*texture;

	t_vec4_list		*vertices_in_world; // utils for gravity
...
t_mesh			create_primitive_cube(t_vec4 coord, t_vec4 size, char *texture_path, float gravity);
t_mesh			create_primitive_plane(t_vec4 pos, t_vec4 size, char *texture_path, float gravity);
t_mesh			create_primitive_vertical_plane(t_vec4 pos, t_vec4 size, char *texture_path, float gravity);
t_mesh			create_primitive_skybox(t_vec4 pos, t_vec4 size, t_texture *p_texture);
t_mesh			create_primitive_cave(t_vec4 pos, t_vec4 size, char *texture_path, float gravity);
void			t_mesh_rotate(t_mesh *mesh, t_vec4 delta_angle);
void			t_mesh_rotate_to(t_mesh *src, t_mesh *dest);
void			t_mesh_rotate_around_point(t_mesh *mesh, t_vec4 delta_angle, t_vec4 center);
