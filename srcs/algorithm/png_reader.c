...
t_surface		*read_png_file(const char *filename)
{
	...
	if ((fp = fopen (filename, "rb")) == NULL)
		error_exit(-501, ft_strjoin(filename, " doesn't exist"));		//ouverture du fichier .png. "rb" = read byte donc lecture uniquement et en byte.
	...
	png_get_IHDR (png_ptr, info_ptr, (png_uint_32*)(&surface->w),			//Prepare la lecture des pixels en
              		(png_uint_32*)(&surface->h), &bit_depth, &color_type,	//rangeant bien les infos que
	...
	surface->pixels = (GLubyte *)malloc (sizeof (GLubyte) * surface->w
								* surface->h * surface->internalFormat);	//malloc d'un espace suffisant pour tous les pixels
	...
	while (++i < surface->h)
		row_pointers[i] = (png_bytep)(surface->pixels + ((surface->h - (i + 1))
									* surface->w * surface->internalFormat));	//met dans ma ligne de pixels l'adresse des premiers pixels des lignes de surface
