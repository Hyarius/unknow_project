...
t_mesh	*cast_ray_next(t_vector4 pos, t_vector4 direction, t_mesh *mesh)
{
	...
	while (++k < mesh->faces->size)
	{
		if ((mesh->no_hitbox == 0 || mesh->is_visible == 1) &&
			intersect_triangle_by_segment(compose_t_triangle_from_t_mesh
...
t_mesh	*cast_ray(t_engine *engine, t_vector4 pos, t_vector4 dir, char *shooter)
{
	int 		i;
	int 		j;
	t_mesh 		*mesh;
	...
	i = -1;
	dir = divide_vector4_by_float(dir, 10);
	while (++i < 100)
	{
		j = -1;
		while (++j < t_engine_return_mesh_len(engine))
		{
			...
			{
				mesh = cast_ray_next(pos, dir, mesh);
				if (mesh != NULL)
					return (mesh);
			}
		}
		pos = add_vector4_to_vector4(pos, dir);
	}
	return (NULL);
}
