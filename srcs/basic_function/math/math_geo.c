...
/*   Updated: 2019/10/31 17:05:10 by adjouber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector3		intersect_plane_by_line(t_vector3 p_normal, t_vector3 p_center,
											t_vector3 start, t_vector3 end)
{
	...
	tabf[0] = dot_t_vector3(normalized_plane, p_center);
	tabf[1] = dot_t_vector3(start, normalized_plane);
	tabf[2] = dot_t_vector3(end, normalized_plane);
	tabf[3] = (tabf[0] - tabf[1]) / (tabf[2] - tabf[1]);
	basic_line = substract_vector3_to_vector3(end, start);
	intersection = mult_vector3_by_float(basic_line, tabf[3]);
...
float			calc_distance_to_plane(t_vector3 p_normal, t_vector3 p_center,
														t_vector3 p_point)
{
	t_vector3	normalized_plane;
	float		result;
...
t_vector3		interpolate_vector3_over_line(float ratio_x, float ratio_y,
													t_vector3 ua, t_vector3 ub)
{
	t_vector3	result;
