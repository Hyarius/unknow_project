...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < nb; i++)
	{
		...
		mesh_list[i] = initialize_t_mesh(); //creation d'un mesh vide
		*(mesh_list[i]) = create_primitive_cube(create_t_vector3(x, y, z), create_t_vector3(1, 1, 1)); //creation du cube que l'on rentre dans mesh
	}
	...
	t_matrix	*mvp; // Model View Projection
	mvp = initialize_t_matrix();

	while (play == 1)
	{
		*mvp = compute_t_camera(cam); //calcul du mvp
		...
		for (int i = 0; i < nb; i++)
			draw_t_mesh(win, mvp, mesh_list[i]); // dessine le cube
