...
/*   Updated: 2020/01/21 14:47:03 by spuisais         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	shoot_weapon(t_engine *engine, t_camera *cam, t_texture **tex_arme)
{
	t_mesh		*target;
	t_weapon	*weapon;
	float		dist;
		...
		% weapon->tick_shoot == 0)
	{
		if (weapon->ammo > 0)
		{
			...
			tex_arme[engine->user_engine->player->current_weapon->index + 6]);
