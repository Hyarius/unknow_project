...
{
	t_3d_triangle_list list;

	if (!(list.triangle = (t_3d_triangle *)malloc(sizeof(t_3d_triangle) * PUSH_SIZE)))
		error_exit(-32, "Can't malloc a t_3d_triangle array");
	list.size = 0;
...
t_3d_triangle_list *initialize_t_3d_triangle_list()
{
	...
	if (!(list = (t_3d_triangle_list *)malloc(sizeof(t_3d_triangle_list))))
		error_exit(-33, "Can't create a t_3d_triangle_list array");

	*list = create_t_3d_triangle_list();
...
void	t_3d_triangle_list_push_back(t_3d_triangle_list *dest, t_3d_triangle to_add)
{
	...
	{
		tmp = dest->triangle;
		if (!(dest->triangle = (t_3d_triangle *)malloc(sizeof(t_3d_triangle) * (dest->size + 1 + PUSH_SIZE))))
			error_exit(-34, "Can't realloc a t_3d_triangle array");
		i = 0;
		while (i < dest->size)
		{
...
void	t_3d_triangle_list_add_back(t_3d_triangle_list *dest, t_3d_triangle *to_add)
{
	...
	if ((dest->size + 1) >= dest->max_size)
	{
		tmp = dest->triangle;
		if (!(dest->triangle = (t_3d_triangle *)malloc(sizeof(t_3d_triangle) * (dest->size + 1 + PUSH_SIZE))))
			error_exit(-35, "Can't realloc a t_3d_triangle array");
		i = 0;
		while (i < dest->size)
		{
			dest->triangle[i] = tmp[i];
...
t_3d_triangle	t_3d_triangle_list_at(t_3d_triangle_list *dest, int index)
{
	if (index < 0 || index >= dest->size)
		error_exit(-36, "Segfault : t_3d_triangle_list out of range");
...
void			t_3d_triangle_list_resize(t_3d_triangle_list *dest, int new_size)
{
	...
	tmp = dest->triangle;
	if (!(dest->triangle = (t_3d_triangle *)malloc(sizeof(t_3d_triangle) * (new_size))))
		error_exit(-35, "Can't realloc a t_3d_triangle array");
	i = 0;
