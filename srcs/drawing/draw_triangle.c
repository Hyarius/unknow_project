...
void draw_triangle_texture(t_window *p_win, t_triangle *p_t_screen, t_triangle *p_t_uv, t_image *p_texture, float p_alpha)
{
	t_triangle triangle;
	t_vector2 coord[3];

	coord[0] = convert_screen_to_opengl(p_win, (int)(p_t_screen->a.x), (int)(p_t_screen->a.y)); // convertit la coordonnee a l'ecran pour opengl
	...
	triangle = create_t_triangle(coord[0], coord[1], coord[2]); //creation du triangle

	draw_triangle_texture_opengl(p_win, &triangle, p_t_uv, p_texture, p_alpha); //affichage du triangle
}
