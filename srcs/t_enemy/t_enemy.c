...
void		enemy_look(t_engine *engine)
{
	...
	while (++i < engine->visual_engine->camera_list->size)
	{
		...
		if (cam->body != NULL)
		{
			if (ft_strcmp(cam->body->name, "Enemy_basic") == 0)
			{
...
void		enemy_shoot(t_engine *engine)
{
	...
	{
		target = t_mesh_list_get(engine->physic_engine->mesh_list, i);
		if (ft_strcmp(target->name, "Enemy_basic") == 0)
		{
...
void		enemy_move(t_engine *engine)
{
	...
	while (i < engine->physic_engine->mesh_list->size)
	{
		...
		if (ft_strcmp(mesh->name, "Enemy_basic") == 0)
		{
			target = cast_ray(engine, mesh->camera->pos, mesh->camera->forward, "Enemy_basic");
