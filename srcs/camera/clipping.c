...
{
	t_vec4 		outside_points[3];
	t_vec4 		inside_points[3];
	t_vec4 		outside_points_uv[3];
	t_vec4 		inside_points_uv[3];
	t_vec4		forward;
	t_vec4		plane_center;
	t_vec4		*clipping_list;
	t_vec4		*clipping_list_uv;
	int				outside_nb;
	int				inside_nb;
	float			dist[3];
	...
	forward = new_vec4(0, 0, 1);
	plane_center = new_vec4(0, 0, p_camera->near);
	...
	if (inside_nb == 1) // si 1 point a l'interieur, creation de 2 nouveaux point pour refaire le triangle
	{
		...
		if (p_points_uv != NULL)
		{
			...
			clipping_list_uv[1] = interpolate_vec4_over_line(
			...
			clipping_list_uv[2] = interpolate_vec4_over_line(
	...
	else if (inside_nb == 2) // si 2 points a l'interieur, creation de 2 nouveaux point pour creer 2 triangles
	{
		...
		if (p_points_uv != NULL)
		{
			...
			clipping_list_uv[4] = interpolate_vec4_over_line(
											...
											inside_points_uv[0], outside_points_uv[0]);
			clipping_list_uv[5] = interpolate_vec4_over_line(
