...
static void 	draw_scan_line(t_window *p_win, t_vector3 left, t_vector3 right)
{
	int pixel_index;
	float	ratio;
	t_color	color;
	...
	while (left.x <= right.x)
	{
		if (left.x + 0.5 >= 0 && left.x - 0.5 < p_win->size_x && left.y + 0.5 >= 0 && left.y - 0.5 < p_win->size_y)
		{
			...
			{
				ratio = (left.z - dist_max) / (0 - dist_max);
				color.r = ratio;
				color.g = ratio;
				color.b = ratio;
				color.a = 1.0f;
				draw_pixel(p_win, (int)(left.x), (int)(left.y), &color);
...
static void	fill_down_flat_triangle(t_window *p_win, t_triangle *p_triangle)
{
	t_vector3	target;
	t_vector3	left;
	t_vector3	right;
	...
	{
		draw_scan_line(p_win, left, right);
		left = substract_vector3_to_vector3(left, delta_left);
		right = substract_vector3_to_vector3(right, delta_right);
	}
}

static void	fill_top_flat_triangle(t_window *p_win, t_triangle *p_triangle)
{
	t_vector3	target;
	t_vector3	left;
	t_vector3	right;
...
void	draw_triangle_depth_cpu(t_window *p_win, t_triangle *p_triangle)
{
	...
	if (p_triangle->b.y == p_triangle->c.y)
		fill_down_flat_triangle(p_win, p_triangle);
	else if (p_triangle->a.y == p_triangle->b.y || p_triangle->a.y == p_triangle->c.y)
		fill_top_flat_triangle(p_win, p_triangle);
	else
	{
		...
		fill_down_flat_triangle(p_win, p_triangle);
		fill_top_flat_triangle(p_win, &tmp);
	}
}
